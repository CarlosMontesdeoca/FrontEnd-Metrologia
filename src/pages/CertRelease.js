...
export default function CertRelease( {token, url } ) {
    let date = new Date();
    ...
    const getCertificates = async (id) => {
        const data = await fetch(`${url}projects/${id}/certificates/to-A`, {
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            }
    ...
    const handleLiberate = () => {
        Swal.fire({
            title: '¿Desea Liberar el certificado?',
            ...
            if (confirm.isConfirmed) {
                ...
                Swal.fire('El Certificado a sido lberado', '', 'success')
                .then(() => {
                    window.location.reload()
                })
            }
    ...
    return(
        <section className="release">
            ...
            <h2 className='title'> PROYECTOS Liberar </h2>
            ...
            { projects.length === 0 ? (
                ...
                <>{ page === '' ? (
                    ...
                    <button className="btn-ok btn-stl"  onClick={handleLiberate}> Enviar para Aprovar  <Icon icon="flat-color-icons:approval" height="30"/></button>
                    </>
                )} </>
            )}
